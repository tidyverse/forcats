...
test_that("can collapse un-named levels to Other", {
  f1 <- factor(letters[1:3])
  f2 <- expect_warning(fct_collapse(f1, xy = c("a", "b"), group_other = TRUE), "`group_other` is deprecated")
...
test_that("collapses levels correctly when group_other is TRUE but no other variables to group", {
  ...
  f2 <- expect_warning(fct_collapse(f1, x1 = c("a", "b", "d"), x2 = "c", group_other = TRUE), "Unknown levels|`group_other` is deprecated")
...
test_that("collapses levels correctly when group_other is TRUE and some Other variables to group", {
  f1 <- factor(letters[1:4])
  f2 <- expect_warning(fct_collapse(f1, x1 = c("a", "d"), x2 = "c", group_other = TRUE), "`group_other` is deprecated")
...
test_that("does not use Other for other_level when group_other = TRUE and other_level is not NULL", {
  f1 <- factor(letters[1:4])
  f2 <- fct_collapse(f1, x1 = c("a", "d"), x2 = "c", other_level = "Something else", group_other = TRUE)

  expect_equal(f2, factor(c("x1", "Something else", "x2", "x1"), levels = c("x1", "x2", "Something else")))
})
