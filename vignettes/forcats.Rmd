...
```{r fct-infreq-hair}
#| fig.alt: >
#|   The A bar chart of hair color, now ordered so that generally the least
...
#|   NAs are at the top of the graph, even though there are ~5 NAs and other
#|   colors have smaller values.
...
To understand why, we need to make a brief digression to discuss implicit vs explicit `NA`s in factors.

## Implicit vs explicit missing values

There are two ways to representing a missing value in a factor which we call implicit and :

-   An **implicit** missing value does not appear in the levels and `is.na()` reports it as missing. This is the usual way to encode a missing value in a factor:
...
-   An **explicit** missing appears in the levels and `is.na()` does not report it as missing. This requires a little more work to create:
...
Implicit `NA`s tend to be best for data analysis, since `is.na()` works as you'd expect.
Explicit `NA`s are often better for display since you can control where they appear.

So to fix the issue above, we can use `fct_explicit_na()` to convert the implicit missing values above to explicit.
Then they appear where you'd expect:

```{r}
#| fig.alt: >
#|   The A bar chart of hair color, now ordered so that NAs, now labelled
#|   "(missing)" are ordered where you'd expect: in between white (4) and
#|   black (12).
ggplot(starwars, aes(y = fct_infreq(fct_explicit_na(hair_color)))) + 
  geom_bar() + 
  labs(y = "Hair color")
```
